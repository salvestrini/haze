...
void Semaphore::signal()
{
        if (SDL_SemPost(semaphore_)) {
                throw std::runtime_error(std::string("Unable to signal "
                                                     "semaphore, ") +
                                         SDL_GetError());
        }
}
...
void Mutex::lock()
{
        if (SDL_LockMutex(mutex_)) {
                throw std::runtime_error(std::string("Unable to lock "
                                                     "mutex, ") +
                                                     SDL_GetError());
        }
}
...
void Mutex::unlock()
{
        if (SDL_UnlockMutex(mutex_)) {
                throw std::runtime_error(std::string("Unable to unlock "
                                                     "mutex, ") +
                                                     SDL_GetError());
        }
}
